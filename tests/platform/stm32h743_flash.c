...
int main(void)
{
    ...
    results[6] = (nvm_devErase(&flash_driver_128k, sector11, 16384)
                  == -EINVAL); // Erase 16K

    // Erase misaligned address
    results[7] = (nvm_devErase(&flash_driver_128k, sector11 + 1, 131072)
                  == -EINVAL); // off-by-1
    ...
    results[12] = (nvm_devErase(&flash_driver_128k, sector11 + 0x20000, 0x20000)
                   == 0);
