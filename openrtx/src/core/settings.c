 ...
 * @return 0 if successful, negative error code otherwise
 */
int parse_partition(const int dev, const int part_nb, size_t *offset,
                    size_t limit)
{
    uint8_t buffer[6];
...
/**
 * Finds the latest valid store from a partition along with the offset where to find it
 *
 * @param dev NVM device to read from
 * @param part NVM partition to read from
 * @param store pointer to the store to read to
 ...
 */
int find_latest_valid_store(const int dev, const int part,
                            settings_store_t *store, size_t *offset)
{
    ...
    *offset = 0;
    ...
    while (end_lim > 0) {
        ...
        int ret = parse_partition(dev, part, &read_offset, end_lim);
        if (ret == -EILSEQ) // Corrupt partition
        {
            return 0;
        } else if (ret == -ENOENT) // Empty partition
        {
            return 1;
        } else if (ret < 0) // Error while parsing
            return ret;
        ...
        {
            // Limit the parsing to right before the latest store found
            end_lim -= store->length;
        } else if (ret == -1) // Stale
        {
            return 2;
        }
    ...
    // No valid store, partition is considered corrupted
    return 0;
}
...
int settings_storage_load(settings_storage_t *s, settings_t *settings)
{
    ...
    if (s->initialized) {
        memcpy((void *)settings, (void *)&(s->latest_store.settings),
               sizeof(settings_t));
        return 0;
    } else {
        int ret = 0;
        settings_store_t store_A;
        size_t offset_A;
        bool store_A_stale = false;

        ret = find_latest_valid_store(s->dev, s->part_A, &store_A, &offset_A);
        if (ret < 0)       // Error
            return ret;
        else if (ret == 0) // Part A is corrupted
            s->part_A_status = -1;
        else if (ret == 1) // Part A is empty
        {
            s->part_A_offset = 0;
            s->part_A_status = 0;
        } else if (ret == 2) // Latest store in part A is stale
        {
            ...
            s->part_A_status = 1; // Partition is clean
            store_A_stale = true;
        ...
        settings_store_t store_B;
        size_t offset_B;
        bool store_B_stale = false;
        ret = find_latest_valid_store(s->dev, s->part_B, &store_B, &offset_B);
        if (ret < 0)       // Error
            return ret;
        else if (ret == 0) // Part B is corrupted
            s->part_B_status = -1;
        else if (ret == 1) // Part B is empty
        {
            s->part_B_offset = 0;
            s->part_B_status = 0;
        } else if (ret == 2) // Latest store in part B is stale
        {
            ...
            s->part_B_status = 1; // Partition is clean
            store_B_stale = true;
        ...
        if (s->part_A_status <= 0)     // Part A is corrupt or empty
        {
            ...
            // If part B is corrupt or empty, this is handled at the end of the function
        } else                         // Part A is clean
        {
            if (s->part_B_status == 1) // Part B is clean
            {
...
int settings_storage_save(settings_storage_t *s, const settings_t *settings)
{
    ...
    if (s->write_needed) {
        if (s->latest_store.counter % 2) // Write to part B
        {
            ...
            if (ret < 0)
                return ret;
            s->part_B_status = 1; // Partition is now clean
        } else                    // Write to part A
        {
            ...
            if (ret < 0)
                return ret;
            s->part_A_status = 1; // Partition is now clean
        }
    }
