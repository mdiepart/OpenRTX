                     ...
                     const size_t len)
{
    ...
    {
        if ((address + len) > (FLASH_BASE + (flash_size() << 10)))
            return -EINVAL; // Above address space
    } else if (address + len > config->address_high)
        return -EINVAL;     // Above address space
                         ...
                         const void *data, size_t len)
{
    ...
    if (flash_write(address, data, len))
        return 0;
    return -EIO;
}
...
static int nvm_api_erase(const struct nvmDevice *dev, uint32_t address,
                         size_t len)
{
    const struct STM32FlashArea *config =
        (const struct STM32FlashArea *)(dev->priv);
    int ret = STM32Flash_check(dev, address, len);
    if (ret < 0)
        return ret;
